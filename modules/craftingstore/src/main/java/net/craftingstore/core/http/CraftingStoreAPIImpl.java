...
public class CraftingStoreAPIImpl extends CraftingStoreAPI {
    ...
    public CraftingStoreAPIImpl(CraftingStore instance) {
        ...
        try {
            ...
            instance.getLogger().error("Unable to initialize CraftingStore HTTPS client: " + e.getMessage());
        }
    }
            ...
            try {
                ...
                try {
                    ...
                    if (status < 200 || status >= 300) {
                        instance.getLogger().error("Complete Donations call returned HTTP " + status);
                    }
                } finally {
                    response.close();
                }
            } catch (IOException e) {
                instance.getLogger().error("Complete Donations call failed: " + e.getMessage());
                if (instance.getLogger().isDebugging()) e.printStackTrace();
            }
        });
    }
    ...
    public Future<Boolean> createPayment(
        ...
        return executor.submit(() -> {
            try {
                ...
                instance.getLogger().error("Payment create call failed: " + e.getMessage());
                if (instance.getLogger().isDebugging()) e.printStackTrace();
                return false;
            }
        });
    ...
    public void shutdown() {
        ...
        if (httpClient != null) {
            try {
                ...
                instance.getLogger().error("Failed to close CraftingStore HTTP client: " + exception.getMessage());
            }
        }
    }
